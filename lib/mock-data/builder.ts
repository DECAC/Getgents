...
7. Reste concis, professionnel et orienté action (propose des relances ou affinages).`,
  ...
  // Gent de référence « jeu de rôle », VERSION GÉNÉRATIVE — le jeu tenu par un
  // MODÈLE, à comparer avec `elysee-2027-deterministe` juste après, qui tient
  // le même jeu par du code. Conservé volontairement : c'est le seul point de
  // comparaison entre les deux façons de faire (coût, latence, exactitude des
  // jauges, variété des situations). Tout repose ici sur
  ...
  "elysee-2027": {
    id: "elysee-2027",
    name: "Élysée 2027 — Dans la peau du président (génératif)",
    icon: "🏛️",
    objective:
...
- Premier message d'une partie : accueil en deux phrases (le contexte du mandat), tableau de bord de départ, première situation, première question. Si le joueur écrit librement au lieu de choisir, réponds en une phrase puis repose la question avec ses options.`,
  ...
  // Gent de référence « jeu de rôle », VERSION DÉTERMINISTE — jumeau de
  // `elysee-2027` ci-dessus, gardé à côté de lui POUR COMPARER les deux façons
  // de tenir un même jeu : les règles dans le code, ou les règles dans le
  // prompt. Ici, le mécanisme ne
  // repose plus sur aucun modèle de langage. Les réponses sont produites par
  // le moteur de règles de `lib/elysee2027/` (50 décisions écrites à l'avance,
  // issues des rapports de la Cour des comptes 2026, de l'INSEE 2025 et de la
  // Banque de France 2025 ; 4 options par décision, effets chiffrés sur les
  // 5 jauges, fins de partie imposées par le code). Le marqueur `moteurJeu`
  // déroute le chat vers ce moteur : ni clé OpenRouter, ni quota, ni dérive
  // des jauges — et le choix libre « Autre » est exclu des questions.
  "elysee-2027-deterministe": {
    id: "elysee-2027-deterministe",
    name: "Élysée 2027 — Dans la peau du président (déterministe)",
    icon: "🏛️",
    objective:
      "Jeu de rôle : incarner le prochain président de la République et prendre des décisions rapides dont chaque conséquence, chiffrée et sourcée, fait bouger le bonheur et la confiance des Français.",
    // Ce prompt n'est PAS utilisé en jeu : `moteurJeu` court-circuite le
    // modèle. Il documente l'intention pour qui ouvre l'onglet Prompt & Modèle.
    systemPrompt:
      "« Élysée 2027 » est un jeu DÉTERMINISTE : il ne fait appel à aucun modèle de langage. " +
      "Les 50 décisions, leurs conséquences et les jauges sont définies dans le code (lib/elysee2027/), " +
      "à partir de chiffres publics sourcés (Cour des comptes, INSEE, Banque de France). " +
      "Fiction non partisane : aucune personnalité ni parti réel, aucune consigne de vote.",
    status: "draft",
    updatedAt: "aujourd'hui",
    modelAssignments: [
    ...
    knowledgeSources: [],
    connectors: [],
    moteurJeu: "elysee-2027",
    starters: ["Nouvelle partie"],
    jumpForm: {
      id: "jump-elysee-2027-deterministe",
      title: "Nouvelle partie",
      description: "Vous venez d'être élu(e). Choisissez le contexte de votre mandat.",
      submitLabel: "Entrer à l'Élysée",
      fields: [
    ...
    builderConversation: [
      {
        role: "agent",
        text: "<p>Gent de référence « jeu de rôle », version <b>déterministe</b> : le joueur incarne le président et tranche 50 décisions possibles écrites à l'avance — chacune présente 4 options aux effets chiffrés sur 5 jauges, jusqu'à la victoire ou la défaite (révolution, guerre civile, mise sous tutelle).</p><p>Ce gent n'appelle <b>aucun modèle d'IA</b> : les situations et les chiffres viennent des rapports publics (Cour des comptes 2026, INSEE 2025, Banque de France 2025), les jauges sont exactes, et une partie ne coûte rien. Testez une partie courte en Preview — le formulaire de départ choisit difficulté, priorité et durée.</p>",
        t: "aujourd'hui",
      },
    ],
  },
